...
var arraySum = function(array) {
    ...
    for (var i = 0; i < array.length; i++){
        var current = array[i]; // Current index in array
        if (Array.isArray(current)){
            sum += arraySum(current); // We must go deeper!
        }
        else{
            sum += current;
        }
    ...
    return sum;
    // If it is an array call this function on the current index
    // If it isn't an array, add the value to sum
    // Return
...
// Game plan:  We're going to subtract two from the number by calling this function repeatedly until we get zero
// ... or negative one.
// The base case will be if we hit 0.  If so, return true.  If we hit <= 1, it's not going to be even.
